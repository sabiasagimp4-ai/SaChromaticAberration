...
- 標準は「絶対値（反転なし）」です。倍率は `abs(1 + スケール/100 × 距離係数 × 波長位置)` で、負の倍率による反転を防ぎます。倍率0では中心を参照し、その先は正の倍率へ折り返します。指数方式は引き続き選択できます。
- 半径・減衰は収差・ラジアル・スケールのすべてに常時適用します。適用先の選択肢はありません。中心を鮮明に保って周辺を変形するには、減衰を正の値にします（冪乗の減衰0は均一です）。
